...
const playAudio = (source, loop) => {                 //function to provide what audio file to play on start & end of game
  audio.src = source;
  audio.autoplay = true;
  audio.loop = loop;
};
...
const startGame = () => {
  ...
  document.querySelectorAll(".bulb").forEach((element) => {        // adding eventlistner for each of the bulbs
    element.addEventListener("click", checkBird);                                                        
  }); 
  
   intervalId = setInterval(() => {                            //for keep changing the bulbs in every 1 sec
    document.querySelectorAll(".bulb").forEach((element) => {       //hiding away all the birds
      element.style.setProperty("content",""); ;
    });
    var d = Math.floor(Math.random() * 4) + 1;                   // get random number between 1 and 4;
    document.getElementById("bulb" + d).style.setProperty("content","url(./image.png)"); 
    birdCount++;
    if(birdCount>hitCount+3){                        // After missed three highlighted bulbs game ends
      endGame();
    } 
  }, 1000);
};
...
const checkBird = (event) => {                  //to determine source of the event(which button is clicked)
